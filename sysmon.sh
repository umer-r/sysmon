...
dns_=$(cat /etc/resolv.conf | grep 'name' | head -n 1 | awk {'print $2'}); if [[ $dns_ == "" ]]; then dns_="resolv.conf not Found!"; fi
...
uuid=$(cat /etc/machine-id 2>/dev/null); if [[ $uuid == "" ]]; then uuid="machine-id not Found!"; fi
frwl=$(systemctl status firewalld 2>/dev/null | grep Active | awk {'print $2"::"$3'} || systemctl status ufw 2>/dev/null | grep Active | awk {'print $2"::"$3'}); if [[ $frwl == "" ]]; then frwl="Firewall not Found!"; fi
echo "+-----------------------+"
